...
Set the environment variables `DB_NAME`, `DB_HOST`, `MYSQL_PORT`,
`DB_USER` and `MYSQL_PASSWORD`. `MYSQL_SOCKET` can also be used in place
of `DB_HOST` and `MYSQL_PORT` to connect over a UNIX socket. Then run
`npm test`.
...
$ DB_HOST=localhost MYSQL_PORT=3306 DB_NAME=node_mysql_test DB_USER=root MYSQL_PASSWORD= FILTER=integration npm test
```
